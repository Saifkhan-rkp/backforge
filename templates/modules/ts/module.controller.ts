...
import { TestServices } from './module.service';
...
export const deleteTest = catchAsync(async (req: Request, res: Response) => {
    ...
    const isDleted = result.deletedCount > 0
    sendResponse(res, {
        success: isDleted,
        statusCode: isDleted ? httpStatus.OK : httpStatus.NOT_MODIFIED,
        message: `Requested Document ${isDleted ? "" : "not"} deleted`
    })
})